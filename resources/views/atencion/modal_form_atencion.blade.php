...
<div class="modal fade" id="modal_mante" tabindex="-1" role="dialog" aria-labelledby="largemodal" aria-hidden="true">
		<div class="modal-dialog modal-lg " role="document">
			<div class="modal-content">
				...
				<form class="form-horizontal" action="{{ route('guardar.atencion') }}" method="POST">
					@csrf
				<div class="modal-body"> 
					...
					<div class="form-group row">
						...
						<div class="col-md-3">
							<input type="date" name="fch_pedido" class="form-control"  required="" value="{{ @$info->fch_pedido }}">
						</div> 
						<label class="col-md-3 form-label">Hora Pedido</label>
						<div class="col-md-3">
							<input type="time" name="hora_pedido" class="form-control"  value="{{ @$info->hora_pedido }}">
						</div>
					</div>
					<div class="form-group row">
						<label class="col-md-3 form-label">Fecha atención</label>
						<div class="col-md-3">
							<input type="date" name="fch_entrega" class="form-control"   value="{{ (@$info->fch_entrega!='')?@$info->fch_entrega:date('Y-m-d') }}">
						</div>
						<label class="col-md-3 form-label">Hora Atención</label>
						<div class="col-md-3">
							<input type="time" name="hora_entrega" class="form-control"  value="{{ (@$info->hora_entrega!='')?@$info->hora_entrega:date('H:i') }}">
						</div>
					</div>
					...
					<div class="form-group row">
						...
						<div class="col-md-7">
							<select  id='encargado_lab_id' name="encargado_lab_id"  style="width: 100%;" class="select2AjaxPersona">
								@if(@$info->encargado_lab_id>0)
									<option selected value="{{ $info->encargado_lab_id }}">{{ $funciones->info_persona($info->encargado_lab_id) }}</option>
								@endif
							</select>	
					...
					<div class="form-group row">
						...
						<div class="col-md-7">
							<select  id='recibido_id' name="recibido_id"  style="width: 100%;" class="select2AjaxPersona">
								@if(@$info->recibido_id>0)
									<option selected value="{{ $info->recibido_id }}">{{ $funciones->info_persona($info->recibido_id) }}</option>
								@endif
							</select>	
					...
					<div class="form-group row">
						...
						<div class="col-md-7">
							<select  id='solicitado_id' name="solicitado_id"  style="width: 100%;" class="select2AjaxPersona">
								@if(@$info->solicitado_id>0)
									<option selected value="{{ $info->solicitado_id }}">{{ $funciones->info_persona($info->solicitado_id) }}</option>
								@endif
							</select>				
						</div>
						<div class="col-md-2">
					...
					<div class="form-group row">
						...
						<div class="col-md-3">
							<select name="cargo_solicitado" class="form-control" required="" >
								...
								<option @if($cargo->id == @$info->cargo_solicitado) selected @endif value="{{ $cargo->id }}">{{ $cargo->cargo }}</option>
								@endforeach
							</select>
						</div>
						<label class="col-md-3 form-label">Núm. Documento</label>
						<div class="col-md-3">
							<input type="text" name="numdoc_solicitado" class="form-control"  value="{{ @$info->numdoc_solicitado }}">
						</div>
					</div>
					...
					<div class="form-group row">
						...
						<div class="col-md-7">
							<select  id='autorizado_id' name="autorizado_id"  style="width: 100%;" class="select2AjaxPersona">
								@if(@$info->autorizado_id>0)
									<option selected value="{{ $info->autorizado_id }}">{{ $funciones->info_persona($info->autorizado_id) }}</option>
								@endif
							</select>				
						</div>
						<div class="col-md-2">
					...
					<div class="form-group row">
						...
						<div class="col-md-3">
							<select name="cargo_autorizado" class="form-control" required="" >
								<option value="">-- Seleccionar --</option>
								@foreach($m_cargo::orderby('cargo','ASC')->get() as $cargo)
								<option @if($cargo->id == @$info->cargo_autorizado) selected @endif value="{{ $cargo->id }}">{{ $cargo->cargo }}</option>
								@endforeach
							</select>
						</div>
						<label class="col-md-3 form-label">Núm. Documento</label>
						<div class="col-md-3">
							<input type="text" name="numdoc_autorizado" class="form-control"  value="{{ @$info->numdoc_autorizado }}">
						</div>
					</div>
					...
					<div class="form-group row">
						...
						<div class="col-md-7">
							<select  id='resp_atencion_id' name="resp_atencion_id"  style="width: 100%;" class="select2AjaxPersona">
								@if(@$info->resp_atencion_id>0)
									<option selected value="{{ $info->resp_atencion_id }}">{{ $funciones->info_persona($info->resp_atencion_id) }}</option>
								@endif
							</select>
						</div>
						<div class="col-md-2">
