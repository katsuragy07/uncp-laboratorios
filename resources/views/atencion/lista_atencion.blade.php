   ...
   <!-- Modal Nuevo y editar -->
		...
		<div class="modal-dialog modal-dialog-centered text-center" role="document">
			<div class="modal-content tx-size-sm">
				<div class="modal-body text-center p-4">
					...
					<form action="{{ route('borrar.atencion') }}" method="POST">
						@csrf
    ...
    <div class="row justify-content-center">
        <div class="col-lg-12">
            ...
            <div class="card">
                <div class="card-body">
                	<form action="{{ route('listar.atencion') }}" method="GET">
                    <div class="row">
                        <div class="col-lg">
								...
								<?php $i++;  ?>
									<tr>
										...
										<td>
					                        ...
					                        <button class="btn-danger" data-toggle="modal" data-target="#modal_eliminar" onclick="eliminar({{ $fila->id}},'Atención # {{ $fila->id}} <br>Recibido por: {{ $fila->responsable }}')"> 
					                            <i class="glyphicon glyphicon-trash"></i> 
					                        </button>
					                        <a href="{{ url('pdf/ficha_entrega?id='.$fila->id) }}" class="text-info" target="_blank" title="Descargar"><i class="fa fa-file-pdf-o" aria-hidden="true" style="font-size:20px;color:red"></i></a>
										</td> 
									</tr>									
								@endforeach
							</tbody>
...
<script>
   $("#btnnuevo").click(function(event){ 
    ...
    var urlnuevo = "{{ route('mant.atencion') }}";
	...
	$(".editardato").click(function(event){
	    ...
	    var url = "{{ route('mant.atencion') }}?id="+idtablas;
	    $( "#div_ModalMant" ).html('Cargando...');
