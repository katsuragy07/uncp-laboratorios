...
<div class="modal fade" id="modal_mante" tabindex="-1" role="dialog" aria-labelledby="largemodal" aria-hidden="true">
		<div class="modal-dialog modal-lg " role="document">
			<div class="modal-content">
				...
				<form class="form-horizontal" action="{{ route('guardar.docespecifico') }}" method="POST" enctype="multipart/form-data" id="frmproy">
					@csrf
				<div class="modal-body">
					...
					<div class="form-group row">
						...
						<div class="col-md-9">
							<select name="tipo_documento_id" id="tipo_documento_id" required="" class="form-control">
								...
								<option @if($tipodocumento->id == @$info->tipo_documento_id) selected @endif value="{{ $tipodocumento->id }}">{{ $tipodocumento->tipo_doc_especifico }}</option>
								@endforeach
							</select>
						</div>
					</div>
					...
					<div class="form-group row">
						...
						<div class="col-md-9">
							...
							<?php 
								if(@$info->archivo!=''){
								?><a href="files/docespecifico/{{@$info->archivo}}" class="text-info" target="_blank" title="Descargar"><?php  echo @$info->archivo; ?></a>
							<?php }?>
