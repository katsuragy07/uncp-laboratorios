...
<div class="modal fade" id="modal_mante" tabindex="-1" role="dialog" aria-labelledby="largemodal" aria-hidden="true">
		<div class="modal-dialog modal-lg " role="document">
			<div class="modal-content">
				...
				<form class="form-horizontal" action="{{ route('guardar.infoacademica') }}" method="POST" enctype="multipart/form-data" id="frmproy">
					@csrf
				<div class="modal-body">
					...
					<div class="form-group row">
						...
						<div class="col-md-7">
							<select  id='docente_id' name="docente_id"  style="width: 100%;" class="select2AjaxPersona">
								@if(@$info->docente_id>0)
									<option selected value="{{ $info->docente_id }}">{{ $funciones->info_persona($info->docente_id) }}</option>
								@endif
							</select>	
					...
					<div class="form-group row">
						...
						<div class="col-md-4">
							<input type="text" name="hra_academica" autocomplete="off" class="form-control"  value="{{ @$info->hra_academica }}">
						</div>
						<label class="col-xm-2 form-label">Aforo:</label>
						<div class="col-md-4">
							<input type="number" name="aforo" autocomplete="off" class="form-control"  value="{{ @$info->aforo }}">
						</div>
					</div>
					...
					<div class="form-group row">
						...
						<div class="col-md-4">
							<input type="date" required name="fecha_inicio" class="form-control"  value="{{ @$info->fecha_inicio }}">
						</div>
						<label class="col-xm-2 form-label">Fecha Fin:</label>
						<div class="col-md-4">
							<input type="date" required name="fecha_fin" class="form-control"  value="{{ @$info->fecha_fin }}">
						</div>
					</div>
					...
					<div class="form-group row">
						...
						<div class="col-md-9">
							<input type="file" name="calendario_uso" id="calendario_uso" accept=".pdf" class="form-control"  value="{{ @$info->calendario_uso }} " style="display:none;">
							...
							<?php 
								...
								?><a href="files/infoacademica/{{@$info->calendario_uso}}" class="text-info f_calendario_uso" target="_blank" title="Descargar"><?php  echo @$info->calendario_uso; ?></a>
								<input type="hidden" name="f_calendario_uso" id="f_calendario_uso" value="{{@$info->calendario_uso}}">
					...
					<div class="form-group row">
						...
						<div class="col-md-9">
							<input type="file" name="guia_manual" id="guia_manual"  accept=".pdf" class="form-control"  value="{{ @$info->guia_manual }}" style="display:none;">
							...
							<div class="flex m-2" style="color: #29327f;font-size: 1.1rem;max-width: 100%;" id="val_guia_manual" value="{{ @$info->guia_manual }}"></div>

							<?php 
								if(@$info->guia_manual!=''){
								?><a href="files/infoacademica/{{@$info->guia_manual}}" class="f_guia_manual text-info" target="_blank" title="Descargar"><?php  echo @$info->guia_manual; ?></a>
								<input type="hidden" name="f_guia_manual" id="f_guia_manual" value="{{@$info->guia_manual}}">
								...
								<tbody> 
									...
									@foreach($horario  as $fila)
											<tr> 
												<td> <?php $i; ?></td> 
												<td>
													<select id="dia_semana{{ $i }}" name="dia_semana[]" required="" class="form-control"  >
												...
												<td><input type="time" class="form-control" id="hora{{ $i }}" name="hora[]" required="" value="{{ $fila->hora }}"></td>
												<td><input type="time" class="form-control" id="hora_fin{{ $i }}" name="hora_fin[]" required="" value="{{ $fila->hora_fin }}"></td>
					  ...
					  <div class="form-group row">
						<div class="table-responsive">
							<table id="tb_programa" class="table">
								...
								<tbody> 
									<?php $i1 = 1; ?>	
									@foreach($programa  as $fila1)
											<tr> 
												<td> <?php $i1; ?></td> 
												<td><input type="text" autocomplete="off" class="form-control" id="nom_programa{{ $i1 }}" name="nom_programa[]" required="" value="{{ $fila1->nom_programa }}"></td>
												<td><input type="text" autocomplete="off" class="form-control" id="cod_programa{{ $i1 }}" name="cod_programa[]" required="" value="{{ $fila1->cod_programa }}"></td>
												<td class='info' width='5%' align='center'><a href='javascript:void(0);' class='elimina btn' title = 'Quitar Horario'><i class='glyphicon glyphicon-remove-circle txt-color-red'></i></a></td>
											</tr>
