					...
					<div class="form-group row">
						...
						<div class="col-md-6">
							<select name="tipo_fiscalizado_id" required="" class="form-control" >
								<option value="">Seleccionar...</option>
								@foreach($m_tipo_fiscalizado::get() as $tipo_fiscalizado)
								<option @if($tipo_fiscalizado->id == @$info->tipo_fiscalizado_id) selected @endif  value="{{ $tipo_fiscalizado->id }}">{{ $tipo_fiscalizado->tipo_fiscalizado }}</option>
								@endforeach
							</select>
						</div>
					</div>
					...
					<div class="form-group row">
						...
						<div class="col-md-6">
							<select name="unidad_med_min_id" id="unidad_med_min_id" class="form-control" required="" >
								<option value="">-- Seleccionar --</option>
								@foreach($unidad_medida as $unidad)
								<option @if($unidad->id == @$info->unidad_med_min_id) selected @endif value="{{ $unidad->id }}">{{ $unidad->unidad_medida }}</option>
								@endforeach
							</select>
						</div>
					</div>
					...
					<div class="form-group row">
						<div class="table-responsive">
							<table id="tb_lote" class="table">
								...
								<tr>  
									<td><input type="date" class="form-control" id="fch_fabricacion{{ $i }}" name="fch_fabricacion[]" value="{{ $fila->fch_fabricacion }}"></td>
									<td><input type="date" class="form-control" id="fch_vencimiento{{ $i }}" name="fch_vencimiento[]" value="{{ $fila->fch_vencimiento }}"></td>
									<td><input type="text" class="form-control" id="lote{{ $i }}" name="lote[]" value="{{ $fila->lote }}"></td>
									<td><input type="number" class="form-control" id="cantidad_lote{{ $i }}" name="cantidad_lote[]" required item="{{ $i }}" value="{{ $fila->cantidad_lote }}" onkeyup="sumasubtotal();" onmouseup="sumasubtotal();"><input type="hidden" id="cantidad_lote_min{{ $i }}" name="cantidad_lote_min[]" value="{{ $fila->cantidad_lote_min }}"></td>
