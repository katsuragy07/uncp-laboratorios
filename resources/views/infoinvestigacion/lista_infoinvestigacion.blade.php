...
@section('titulo','Lab. de Investigación')
@section('contenido')
<div class=""> <!-- container -->
...
<div id="div_ModalMant">		
		...
		<div class="modal-dialog modal-dialog-centered text-center" role="document">
			<div class="modal-content tx-size-sm">
				<div class="modal-body text-center p-4">
					...
					<form action="{{ route('borrar_infoinvestigacion') }}" method="POST">
						@csrf
    ...
    <div class="row justify-content-center">
        <div class="col-lg-12">
            ...
            <div class="card">
                <div class="card-body">
                	<form action="{{ route('listado.investigacion') }}" method="GET">
                    <div class="row">
                        <div class="col-md-6">
					...
					<div class="row">
						...
						<div class="col-md-9">
                            ...
                            <input class="form-control mb-5" autocomplete="off" placeholder="Ingrese Campos" type="text" name="txtbuscar" value="{{ $databusqueda->cod_sunedu }}">
                        </div>
						...
						<table class="table table-hover card-table table-vcenter text-nowrap mb-0">
							...
							<tbody>
								...
								<?php $i++;  ?>
									<tr>
										<td>{{ $i }}</td>
										<td>{{ $lista->cod_proyecto }}</td>
										<td>{{ $lista->nom_proyecto }}</td>
										<td>{{ $lista->apellidos .' '.$lista->nombres }}</td>
										<td>{{ $lista->fuente_finan }}</td> 
										<td>{{ $lista->centro_inv }}</td>
										<td>{{ $lista->linea_inv }}</td>
										<td>
										...
										<button class="btn-danger" data-toggle="modal" data-target="#modal_eliminar" onclick="eliminar({{ $lista->id}},'{{ $lista->cod_proyecto }}')"> 
...
<script>
 $("#btnnuevo").click(function(event){ 
    ...
    var urlnuevo = "{{ route('mant_infoinvestigacion') }}";
	...
	$(".editardato").click(function(event){
	    ...
	    var url = "{{ route('mant_infoinvestigacion') }}?id="+idtablas;
	    $( "#div_ModalMant" ).html('Cargando...');
   ...
   $(".agregardato").click(function(event){
    	event.preventDefault();     
	    var idtablas = $(this).attr("value");
	    var url = "{{ route('agre_infoinvestigacion') }}?id="+idtablas;
	    $( "#div_ModalMant" ).html('Cargando...');
	    $( "#div_ModalMant" ).load(url);
   }); 
